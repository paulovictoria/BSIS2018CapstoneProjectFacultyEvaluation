...
$teachers = mysql_query("SELECT *, (SELECT SUM(suptotal) FROM details WHERE details.teacherid = teacher.teacherid AND details.semester = '".$_POST['semester']."' AND details.rater = 'category1' AND details.sy = '".$_POST['schoolyear']."') AS supTotal, 
			...
			{
$teachers = mysql_query("SELECT *, (SELECT SUM(suptotal) FROM details WHERE details.teacherid = teacher.teacherid) AS supTotal, 
					   (SELECT SUM(s1) FROM details WHERE details.teacherid = teacher.teacherid) AS s1, 
					   (SELECT SUM(s2) FROM details WHERE details.teacherid = teacher.teacherid) AS s2, 
					   (SELECT SUM(s3) FROM details WHERE details.teacherid = teacher.teacherid) AS s3, 
					   (SELECT SUM(s4) FROM details WHERE details.teacherid = teacher.teacherid) AS s4, 
					   (SELECT SUM(s5) FROM details WHERE details.teacherid = teacher.teacherid) AS  s5,
					   (SELECT COUNT(*) FROM details WHERE details.teacherid = teacher.teacherid) AS evaluateCount FROM teacher group by fname");
			}			
