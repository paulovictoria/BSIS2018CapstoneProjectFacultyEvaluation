...
<title>Rank | Outstanding Performance</title>
...
$teachers2 = mysql_query("SELECT *, (SELECT SUM(supTotal) FROM details WHERE details.teacherid = teacher.teacherid AND details.role = 'superior') AS supTotal, 
			...
			{	
			?>
			<?php $total = ($teacher['aveTotal'] != "") ? $teacher['aveTotal'] / $teacher['evaluateCount'] * 60 / 100 + number_format((float)$teacher2['supTotal'] / $teacher2['evaluateCount1'] * 40 / 100 ,2,".",'') : 0; ?>
			<?php if($total >= 4.2 && $total <= 5 && $teacher['aveTotal'] !=""){ 
				$rank++;
				?>
			<tr>
				<td><?php echo $rank; ?></td>
